...
class FDMain:
    ...
    def syncTemplates(self):
        ...
        for template in json_data:
            ...
            if sha not in self.templates_hash.keys():
                self.debug("发现新模板: {}<br>大小: {} Bytes<br>下载链接: {}".format(name,
                                                                          file_size,
                                                                          download_url))
            else:
                self.debug("模板已存在: {}, 跳过同步".format(self.templates_hash.get(sha)), type='warn')

        self.debug("模板同步完成", type='success')
            ...
            for file_name in file_list:
                ...
                if not file_name.find(".json") == -1:
                    ...
                    with open(os.path.abspath(path) + "\\" + file_name, 'rb') as load_json:
                        ...
                        self.debug(display_name + " [" + hash_value + "]")
